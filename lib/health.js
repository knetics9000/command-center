...
export async function refreshHealth() {
  ...
  const tryGet = async (key, type, days, mode, xform) => { try { const v = await rollup(o, type, days, mode); out[key] = v == null ? null : (xform ? xform(v) : v); } catch { out[key] = null; } };
  await tryGet("steps", "steps", 1, "first");
  await tryGet("restingHr", "dailyRestingHeartRate", 2, "first");
  await tryGet("activeMinutes", "activeZoneMinutes", 1, "sum");
  await tryGet("sleepHours", "sleep", 2, "max", (v) => (v > 3600 ? +(v / 3600).toFixed(1) : v > 60 ? +(v / 60).toFixed(1) : v)); // seconds or minutes -> hours
  ...
  for (const t of ["steps", "dailyRestingHeartRate", "activeZoneMinutes", "sleep"]) {
    try {
      const res = await fetch(`https://health.googleapis.com/v4/users/me/dataTypes/${t}/dataPoints:dailyRollUp`, {
        method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ range: civilRange(4), windowSizeDays: 1, pageSize: 14, dataSourceFamily: "users/me/dataSourceFamilies/all-sources" }),
        signal: AbortSignal.timeout(12000),
      });
