...
import { createEvent, deleteEvent, listEvents, spamMsg } from "./google.js";
import { BUCKETS } from "./buckets.js";

/** Surface already-flagged risky/spammy unhandled emails as one-click cleanup items. */
function detectSpam() {
  const rows = getDb().prepare("SELECT id,account,sender,subject,risk_why FROM emails WHERE handled=0 AND risk=1 ORDER BY datetime(received_at) DESC LIMIT 12").all();
  return rows.map((e) => ({ kind: "spam", emailId: e.id, account: e.account, sender: e.sender, subject: e.subject, why: e.risk_why || "Flagged as likely phishing or spam." }));
}
export { BUCKETS };
...
const sig = (s) => {
  ...
  if (s.kind === "project") return "proj:" + (s.name || "").toLowerCase().replace(/\s*project\s*$/i, "").trim();
  if (s.kind === "calendar") return "cal:" + (s.title || "").toLowerCase().replace(/[^a-z0-9]+/g, "").slice(0, 20) + ":" + (s.date || "");
  if (s.kind === "caldup") return "caldup:" + (s.drop && s.drop.id || "");
  if (s.kind === "spam") return "spam:" + (s.emailId || "");
...
export async function generateCleanup() {
  ...
  // fourth pass: surface likely-spam emails for one-click removal
  try { for (const sp of detectSpam()) suggestions.push(sp); } catch {}
  ...
  // projects that already exist must never be re-suggested for creation
  const projNorm = (x) => (x || "").toLowerCase().replace(/\s*project\s*$/i, "").trim();
  const existingProjects = new Set(db.prepare("SELECT name,tag FROM projects").all().flatMap((r) => [projNorm(r.name), projNorm(r.tag)]).filter(Boolean));
  ...
  for (const s of suggestions) {
    ...
    if (s.kind === "project" && existingProjects.has(projNorm(s.name))) continue;
    const g = sig(s);
...
async function detectCalendarItems(tasks) {
  ...
  const onCal = db.prepare("SELECT title, start FROM calendar_events").all().map((r) => ({ nt: norm(r.title), d: (r.start || "").slice(0, 10) }));
  const onCalHit = (c) => { const cn = norm(c.title); return !!cn && onCal.some((k) => k.d === c.date && k.nt && (k.nt.includes(cn.slice(0, 10)) || cn.includes(k.nt.slice(0, 10)))); };
  ...
  return items.filter((c) => c && c.title && /^\d{4}-\d{2}-\d{2}$/.test(c.date || "") && c.date >= today && !onCalHit(c));
}

const ORDER = "CASE kind WHEN 'spam' THEN 0 WHEN 'caldup' THEN 1 WHEN 'calendar' THEN 2 WHEN 'project' THEN 3 WHEN 'duplicate' THEN 4 WHEN 'mistag' THEN 5 WHEN 'uncategorized' THEN 6 ELSE 7 END";
...
export async function acceptSuggestion(id) {
  ...
  if (s.kind === "mistag" || s.kind === "uncategorized") {
    ...
    const e = db.prepare("SELECT account FROM emails WHERE id=?").get(s.emailId);
    await spamMsg(s.account || (e && e.account), s.emailId);
    db.prepare("UPDATE emails SET handled=1, handled_state='spam', snooze_until=NULL WHERE id=?").run(s.emailId);
  }
