...
export function dismissNotification(id) {
  const db = getDb();
  const n = db.prepare("SELECT app,flagged FROM notifications WHERE id=?").get(id);
  if (n && n.flagged) recordFeedback("notif", n.app, -1);   // AI flagged it, Kurt didn't care → learn
...
export async function analyzeNotifications(max = 15) {
  ...
  const fb = feedbackMap("notif");   // learned per-app preferences from Kurt's past actions
  ...
  rows.forEach((r, idx) => {
    ...
    let flagged = (o.flagged === true || o.flagged === "true" || imp >= 80) ? 1 : 0;
    const score = fb[(r.app || "").toLowerCase()] || 0;   // Kurt's correction history overrides the AI
    if (score <= -2) flagged = 0;
    else if (score >= 2) flagged = 1;
...
export async function notificationToTask(id) {
  ...
  recordFeedback("notif", n.app, 2);   // Kurt acted on it → this app matters
