...
export async function analyzeNotifications(max = 15) {
  ...
  try {
    const out = await askJSON({
      model: FAST,
      system: "You triage Kurt's incoming phone notifications for his ADHD command center. FLAG only what genuinely needs his attention or action: a real person messaging him (especially urgent wording like 'call me'), a time-sensitive alert, money/security/banking, an appointment or delivery that needs a response. Do NOT flag marketing, promos, social reels, video recommendations, newsletters, or app spam.",
      prompt: `Notifications (app, title — body):\n${list}\n\nReturn ONLY this JSON, one entry per item in the same order:\n{"results":[{"i":1,"category":"message|alert|finance|calendar|delivery|social|promo|media|app|other","importance":<integer 0-100>,"flagged":<true/false>,"why":"<=8 words on why it matters, or empty>"}]}`,
      max_tokens: 1200,
    });
  ...
  rows.forEach((r, idx) => {
    ...
    upd.run((o.category || "other").toString().slice(0, 24), imp, flagged, (o.why || "").toString().slice(0, 80), r.id);
  });
  return rows.length;
}
