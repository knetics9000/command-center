...
import { listThoughts, summaryLines, thoughtsCount } from "./sync-thoughts.js";
...
export async function generateBriefing({ primary = false } = {}) {
  ...
  const tCount = thoughtsCount();
  const recentThoughts = listThoughts(2).map((t) => ({ bullet: summaryLines(t.summary)[0] || (t.retextualized_text || "").slice(0, 90), source: t.source }));
  ...
  const payload = {
    ...
    thoughts: recentThoughts.map((t) => t.bullet),
  };

  const prompt = `Kurt's open tasks, unhandled emails, kid co-parenting items, flagged phone alerts, and recent reflections as JSON:

${JSON.stringify(payload)}

"kids" are time-sensitive items about his children (school/medical/schedule) and "alerts" are flagged phone messages — treat these as high-signal and weave the most important into the greeting and priorities. "thoughts" are recent reflections, NOT action items — only nod to one in the greeting if it's clearly relevant; never turn a thought into a priority.
...
Rules: 3-6 priorities, most important first. At most 6 clusters, each with 2+ related items; cap taskIds and emailIds at 8 each. Set suggestProject true when the cluster is an ongoing initiative worth tracking as a project. Use the real ids from the input. Return ONLY the JSON object, no markdown.`;
  ...
  data.thoughts = { count: tCount, recent: recentThoughts };
...
export function briefingToHtml(data, dateLabel) {
  ...
  const healthLine = fam.health
    ...
    : "";
  const th = data.thoughts || {};
  const thoughtsLine = th.count > 0
    ? `<div style="margin-top:12px;font-size:12px;color:#9b9384">💭 ${th.count} recent thought${th.count > 1 ? "s" : ""}${(th.recent || []).length ? " · " + esc((th.recent || []).map((t) => t.bullet).filter(Boolean).join(" · ")) : ""}</div>`
    : "";
