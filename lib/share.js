...
export async function analyzeShare(id) {
  ...
  try {
    out = await askJSON({
      ...
      prompt:
        `Analyze this shared item.\nORIGINAL TITLE: ${row.title || ""}\nURL: ${row.url || ""}\nDOMAIN: ${content.domain || ""}\nFETCHED TITLE: ${content.fetchedTitle || ""}\nDESCRIPTION: ${content.description || ""}\nPAGE TEXT (excerpt): ${(content.text || "").slice(0, 4000)}\nKURT'S NOTE: ${row.text || ""}\n\n` +
        `Kurt's existing project tags (for "projects", choose ONLY from these, 0-3): ${JSON.stringify(projTags)}\n\n` +
        `Return ONE JSON object:\n{"aiTitle":"<clear title describing the REAL value, <=12 words>","categories":["<1-3 content categories like Tutorial, Product Review, AI Development, Business Strategy, Anime Review, Industry News, Podcast>"],"summary":"<2-3 sentence executive summary>","takeaways":["<3-6 key points>"],"insights":["<2-4 actionable insights Kurt can apply>"],"tools":["<tools/products/sites/frameworks mentioned>"],"people":["<people/creators/companies>"],"projects":["<0-3 of Kurt's project tags>"],"credibility":<integer 0-100 estimating source trustworthiness from domain reputation, creator authority, and content quality>,"credReason":"<one short sentence>"} \nReturn ONLY the JSON.`,
      max_tokens: 1600,
    });
  } catch {}
  ...
  db.prepare(`UPDATE shared_items SET ai_title=?, categories=?, summary=?, takeaways=?, insights=?, tools=?, people=?, projects=?, credibility=?, cred_reason=?, analyzed=1 WHERE id=?`).run(
    (out.aiTitle || content.fetchedTitle || row.title || "Untitled").slice(0, 160),
    arr(out.categories), (out.summary || "").slice(0, 800), arr(out.takeaways), arr(out.insights), arr(out.tools), arr(out.people), arr(out.projects),
    Number.isFinite(+out.credibility) ? Math.max(0, Math.min(100, Math.round(+out.credibility))) : null,
    (out.credReason || "").slice(0, 200), id);
}
