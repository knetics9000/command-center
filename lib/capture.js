...
export async function processCapture(id) {
  ...
  try {
    out = await askJSON({
      ...
      prompt: `Brain-dump: "${row.raw_text}"\n\nScore its TRUE importance to Kurt — based on consequences and time-sensitivity, NOT on when it was written. A freshly-typed trivial thought is LOW; an old but critical one is HIGH. Reserve high scores for genuinely urgent or high-impact items.\n\nReturn ONE JSON object:\n{"category":"to-do|idea|errand|email|reminder|note|shopping|event","priority_score":<integer 0-100: 80-100 = urgent or high-impact, 50-79 = normal, 0-49 = minor/someday>,"summary":"<cleaned one-line version of the thought>","suggested_action":"<the concrete next step, imperative, <=10 words>","mood_energy":"<one of: ${MOODS.join(", ")}>"}\nReturn ONLY the JSON.`,
      max_tokens: 300,
    });
  } catch {}
  const mood = MOODS.includes(out.mood_energy) ? out.mood_energy : "quick win";
  const score = Number.isFinite(+out.priority_score) ? Math.max(0, Math.min(100, Math.round(+out.priority_score))) : 55;
  const pri = score >= 70 ? "high" : score >= 40 ? "medium" : "low";
  db.prepare("UPDATE captures SET category=?, priority=?, priority_score=?, summary=?, suggested_action=?, mood_energy=?, status='processed', processed_at=datetime('now') WHERE id=?")
    .run(out.category || "note", pri, score, (out.summary || row.raw_text).slice(0, 300), (out.suggested_action || "").slice(0, 160), mood, id);
  return getOne(id);
}
