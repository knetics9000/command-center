...
function saveTokens(account, email, tokens, picture) {
  const db = getDb();
  const existing = db.prepare("SELECT refresh_token, email, picture FROM google_tokens WHERE account=?").get(account);
  const refresh = tokens.refresh_token || (existing && existing.refresh_token) || null;
  db.prepare(
    `INSERT INTO google_tokens (account,email,picture,access_token,refresh_token,expiry,scope,updated_at)
     VALUES (@account,@email,@picture,@access_token,@refresh_token,@expiry,@scope,datetime('now'))
     ON CONFLICT(account) DO UPDATE SET
       email=excluded.email, picture=excluded.picture, access_token=excluded.access_token,
...
/** Fetch + persist the account's profile photo (and email). Safe to re-run. */
export async function refreshProfile(account) {
  const oa = google.oauth2({ version: "v2", auth: getAuthedClient(account) });
  const d = (await oa.userinfo.get()).data;
  saveTokens(account, d.email, {}, d.picture || null);
  return { email: d.email, picture: d.picture };
}
...
export async function handleCallback(code, account) {
  ...
  let email = null, picture = null;
  try {
    const oa = google.oauth2({ version: "v2", auth: c });
    const d = (await oa.userinfo.get()).data;
    email = d.email; picture = d.picture || null;
  } catch {}
  saveTokens(account, email, tokens, picture);
  return { email };
}
...
export function connectionStatus() {
  return ["personal", "work", "coparent"].map((a) => {
    const r = getTokenRow(a);
    return { account: a, connected: !!(r && r.refresh_token), email: r && r.email, picture: r && r.picture, updated_at: r && r.updated_at };
  });
}
...
export async function listInbox(account, max = 120) {
  ...
  for (const id of ids) {
    ...
    out.push({
      ...
      ts: Number(m.data.internalDate) || null,   // epoch ms — sortable, unlike the Date header
...
/** Arbitrary Gmail search (any folder). Returns light metadata + snippet. */
export async function searchMail(account, query, max = 12) {
  const g = gmail(account);
  ...
  for (const { id } of list.data.messages || []) {
    ...
    out.push({
      id, account,
      from: header(m.data.payload, "From"),
      subject: header(m.data.payload, "Subject"),
      date: header(m.data.payload, "Date"),
      snippet: m.data.snippet || "",
    });
...
export async function getReplyMeta(account, id) {
  const m = await gmail(account).users.messages.get({
    userId: "me", id, format: "metadata", metadataHeaders: ["From", "Subject", "Message-ID", "References"],
  });
  return {
    threadId: m.data.threadId,
    from: header(m.data.payload, "From"),
    subject: header(m.data.payload, "Subject"),
    messageId: header(m.data.payload, "Message-ID"),
    references: header(m.data.payload, "References"),
  };
}
...
/** Send an email FROM and TO Kurt's own account (self-delivery, e.g. the morning briefing). */
export async function sendSelf(account, subject, htmlBody) {
  const row = getTokenRow(account);
  const to = row && row.email;
  if (!to) throw new Error(`No email on file for "${account}"`);
  const lines = [
    `To: ${to}`,
    `Subject: ${subject}`,
  ...
  const res = await gmail(account).users.messages.send({ userId: "me", requestBody: { raw } });
  return res.data;
}

/** Save a reply DRAFT in Gmail (never sends). Threads via In-Reply-To/References. */
export async function createReplyDraft(account, meta, bodyText) {
  const subject = /^re:/i.test(meta.subject || "") ? meta.subject : "Re: " + (meta.subject || "");
  ...
  const raw = b64url(lines.join("\r\n"));
  const res = await gmail(account).users.drafts.create({
    userId: "me", requestBody: { message: { raw, threadId: meta.threadId } },
  });
  return res.data; // { id, message:{id} }
}
...
// ---------- Calendar ----------
export async function createEvent(account, { summary, location, description, start, end, allDay, timeZone = "America/New_York" }) {
  let body;
  if (allDay) {
    const d = (start || "").slice(0, 10);
    const e = end ? end.slice(0, 10) : (() => { const x = new Date(d + "T00:00:00"); x.setDate(x.getDate() + 1); return x.toISOString().slice(0, 10); })();
    body = { summary, location, description, start: { date: d }, end: { date: e } };
  } else {
    body = { summary, location, description, start: { dateTime: start, timeZone }, end: { dateTime: end, timeZone } };
  }
  const res = await calendar(account).events.insert({ calendarId: "primary", requestBody: body });
  return res.data; // has .id (gcal_event_id)
}
export async function deleteEvent(account, eventId) {
  await calendar(account).events.delete({ calendarId: "primary", eventId });
  return { ok: true };
}
