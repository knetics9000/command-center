...
import { recordFeedback, feedbackMap } from "./feedback.js";
...
export async function classifyCoparent(max = 20) {
  const db = getDb();
  const rows = db.prepare("SELECT id,sender,sender_addr,subject,snippet FROM coparent_emails WHERE analyzed=0 ORDER BY received_at DESC LIMIT ?").all(max);
  if (!rows.length) return 0;
  const fb = feedbackMap("kids");   // senders Kurt has promoted before → always important
  ...
  rows.forEach((r, idx) => {
    ...
    let imp = (o.important === true || o.important === "true") ? 1 : 0;
    if ((fb[(r.sender_addr || "").toLowerCase()] || 0) >= 2) imp = 1;   // Kurt promoted this sender before
...
export function promoteKidEmail(id) {
  const db = getDb();
  const row = db.prepare("SELECT sender_addr FROM coparent_emails WHERE id=?").get(id);
  if (row && row.sender_addr) recordFeedback("kids", row.sender_addr, 2);   // teach: this sender matters
  db.prepare("UPDATE coparent_emails SET important=1, seen=0, why=COALESCE(NULLIF(why,''),'Flagged by Kurt'), updated_at=datetime('now') WHERE id=?").run(id);
}
