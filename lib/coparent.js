...
export function kidsBoard() {
  ...
  const db = getDb();
  const rows = db.prepare("SELECT * FROM coparent_emails WHERE analyzed=1 ORDER BY datetime(received_at) DESC LIMIT 300").all();
...
export async function kidEventDraftFromText({ subject, body, kid }) {
  const today = new Date().toISOString().slice(0, 10);
  const out = await askJSON({
    model: FAST,
    system: `You extract a single calendar event from a school/kids email. Today is ${today} (America/New_York).`,
...
export async function kidToCalendar(id) {
  ...
  const draft = await kidEventDraftFromText({ subject: row.subject, body, kid: row.kid });
