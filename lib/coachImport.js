...
export function importConversations(items, { cap = 100 } = {}) {
  ...
  // One transaction: atomic (no partial import on a failed insert) and one WAL commit, not N.
  const ids = getDb().transaction(() =>
    picked.map((item) => insertMaterial({ title: item.title, text: item.text, type: "personal", source: "import" })),
  )();
  for (const id of ids) analyzeGuruMaterial(id).catch(() => {});
