...
export function listPrimeTasks() {
  return getDb().prepare("SELECT id,text,tags,due FROM tasks WHERE status='open' AND prime=1 ORDER BY datetime(updated_at) DESC").all();
}
