...
export async function syncTasks() {
  ...
  // preserve in-app-only fields across the wipe/re-insert (Offload has no due/prime)
  const dueById = new Map(db.prepare("SELECT id,due FROM tasks WHERE due IS NOT NULL").all().map((r) => [r.id, r.due]));
  const primeIds = new Set(db.prepare("SELECT id FROM tasks WHERE prime=1").all().map((r) => r.id));
  const setDue = db.prepare("UPDATE tasks SET due=? WHERE id=?");
  const setPrime = db.prepare("UPDATE tasks SET prime=1 WHERE id=?");
  const tx = db.transaction((rows) => {
    ...
    for (const r of rows) {
      ...
      if (primeIds.has(id)) setPrime.run(id);
    }
...
export async function syncInbox() {
  ...
  const tx = db.transaction(() => {
    for (const m of all) {
      ...
      ins.run({
        ...
        risk: t.risk || 0, risk_why: t.riskWhy || null, category: t.category || null,
      });
    }
  ...
  tx();
  setMeta("hb_mail", new Date().toISOString());
  setMeta("hb_mail_err", failed.length ? failed.join(", ") : "");
