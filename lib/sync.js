...
export async function syncInbox() {
  ...
  const ins = db.prepare(
    `INSERT INTO emails (id,account,thread_id,sender,sender_addr,subject,snippet,received_at,triage_tier,why,action,risk,risk_why,handled,handled_state,updated_at)
     VALUES (@id,@account,@thread_id,@sender,@sender_addr,@subject,@snippet,@received_at,@triage_tier,@why,@action,@risk,@risk_why,0,NULL,datetime('now'))
