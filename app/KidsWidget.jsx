...
export default function KidsWidget() {
  ...
  function gotIt(id) {
    setB((x) => ({ ...x, toReview: Math.max(0, (x.toReview || 1) - 1), kids: x.kids.map((k) => ({ ...k, items: k.items.filter((i) => i.id !== id) })) }));
        ...
        <>
          {b.kids.map((k) => (
            <div className="kidbucket" key={k.name}>
              <div className="kid-h"><span className="kid-name">{k.name}</span><span className="kid-count">{k.items.length ? k.items.length + " to review" : "clear"}</span></div>
              ...
              {k.items.map((e) => (
                <div className="kid-item" key={k.name + e.id}>
                  ...
                  <button className="iconbtn" title="Got it" onClick={() => gotIt(e.id)}><M i="check_circle" /></button>
                </div>
              ))}
            </div>
          ))}
