...
export async function POST(req) {
  ...
  const { url, text, title, influenceId, influenceName } = body;
  if (!url && !text) return NextResponse.json({ ok: false, error: "url or text required" }, { status: 400 });
  const db = getDb();
  const infId = findOrCreateInfluence(db, { id: influenceId, name: influenceName });
  const id = insertMaterial({ url, text, title, influenceId: infId, source: "app" });
